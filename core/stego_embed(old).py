...
def embed_message(cover_img_or_path, edge_map, secret_message, output_path, x=3, y=1):
    ...
    Format bit nhúng:
        [HEADER_LENGTH (32 bits)] [MESSAGE_DATA bits]
    ...
    Returns:
        bits_embedded: tổng số bit đã nhúng (header + message)
    """
    # 1. Text -> bits
    message_bits = text_to_bits(secret_message)
    message_length_bits = len(message_bits)

    # 2. Header 32 bit chứa độ dài message (tính theo bit)
    header_bits = int_to_bits(message_length_bits, 32)

    # 3. Ghép header + data
    secret_bits_full = header_bits + message_bits
...
def extract_message(stego_img, edge_map, x=3, y=1):
    ...
    """
    # 1. Đọc header 32 bit đầu để lấy message_length_bits
    ...
    if message_length_bits <= 0 or message_length_bits > 1_000_000:
        raise ValueError(
            "Độ dài message không hợp lệ! Header có thể đã bị lỗi hoặc "
            "edge map / tham số x,y không khớp lúc nhúng."
        )

    # 2. Đọc lại: 32 (header) + message_length_bits
    ...
    # 3. Bỏ 32 bit header, lấy đúng số bit message
